...
본 장에서는 Amazon EKS 환경에서 폐쇄망으로 Kubespray를 이용하여 HyperCloud를 설치하는 방법에 대해서 설명한다.
...
본 절에서는 EKS 클러스터를 생성 및 사용하기 위한 VPC와 Bastion 서버를 생성하는 방법에 대해서 설명한다.

.EKS를 생성 및 사용하기 위해서는 다음과 같은 사전 준비가 필요하다.
[NOTE]
====
* IAM 계정 발급
* AWS Access Key ID
...
VPC는 AWS Management Console을 통해 생성하거나, AWS CloudFormation 템플릿을 사용하여 자동으로 생성할 수 있다.

본 가이드에서는 AWS Management Console을 사용하여 VPC를 생성하는 방법을 다룬다.

[VPC 서비스 > VPC > VPC 생성] 에서 '생성할 리소스'로 'VPC 등'을 선택하고, '이름'을 입력하고, 환경에 맞게 다른 값들을 설정한다.
...
AWS CLI, kubectl, eksctl 명령을 실행할 Bastion 서버를 생성한다.

. *Bastion 서버 생성*
+
.. 키 페어 생성
+
[EC2 서비스 > 네트워크 및 보안 > 키 페어 > 키 페어 생성] 에서 외부에서 Bastion 서버로 원격 접속하는 데 필요한 한 쌍의 키 페어를 생성한다.
+
private key는 PC에 다운로드 되고, public key는 AWS에 저장된다.
+
image::../../images/eks_create_keypair.png[]
+
.. 보안 그룹 생성
+
[EC2 서비스 > 네트워크 및 보안 > 보안 그룹 > 보안 그룹 생성] 에서 VPC를 설정하고, 인바운드 규칙(유형: SSH, 소스: 내 IP)을 추가하여 보안 그룹을 생성한다.
+
image::../../images/eks_create_sg.png[]
+
.. 인스턴스 생성
+
[EC2 서비스 > 인스턴스 > 인스턴스 시작] 에서 이름을 입력하고, AMI와 인스턴스 유형을 선택한다.
+
키 페어에서 전 단계에서 생성한 키 페어를 선택한다.
+
네트워크 설정에서 편집을 누르고, 생성한 VPC를 선택하고, 서브넷에 Public Subnet을 선택, 퍼블릭 IP 자동 할당을 활성화한다.
+
방화벽(보안 그룹)은 '보안 그룹 생성'을 선택하여 기본으로 입력되어 있는 ssh에 대한 인바운드 규칙을 생성하여 인스턴스를 생성한다.
+
image::../../images/eks_create_bastion.png[]

. *AWS CLI 설치*
+
pem 파일을 사용하여 Bastion의 퍼블릭 IPv4 주소로 접속한다.
+
----
$ ssh -i <pem-file-name>.pem ec2-user@<bastion-public-ip>
----
+
AWS CLI 설치 파일을 다운로드 받고, 압축 해제한 후 설치 프로그램을 실행한다.
+
----
...
본 절에서는 EKS 클러스터를 생성하는 방법에 대해서 설명한다.
...
각 워커 노드에 kubectl 및 .kube/config 파일을 옮긴다.
...
----
{
    "insecure-registries": ["{INTERNAL_IP:PORT}"]
}
----
+
...
ArgoCD 설치를 위해 테크넷을 통해서 다운로드한 *kubespray-eks.zip* 파일을 생성한 bastion 서버에 압축을 해제한다.
...
. *efs 설정*
+
`kubespray-infra/inventory/tmaxcloud/group_vars/k8s_cluster/addons.yml` 파일을 열어 efs를 설정한다.
+
.예시
----
...
. *domain 설정*
+
`kubespray-infra/inventory/tmaxcloud/group_vars/k8s_cluster/k8s-cluster.yml` 파일을 열어 domain을 설정한다.
+
.예시
----
# Enable extra custom DNS domain - by sophal_hong@tmax.co.kr
enable_local_nip_domain: false
...
`kubespray-infra/role/bootstrap-cloud/default/main.yml` 파일을 열어 argocd 설치를 위한 모듈 관련 정보를 설정한다. 해당 파일에서 storageclass, subdomain을 설정할 수 있다.
+
[#KubesprayRunArgoEks]
=== Kubespray 실행
